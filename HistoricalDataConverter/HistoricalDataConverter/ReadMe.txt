================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: HistoricalDataConverter プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した HistoricalDataConverter には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには HistoricalDataConverter アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

HistoricalDataConverter.vcxproj
...
HistoricalDataConverter.vcxproj.filters
...
HistoricalDataConverter.h
   これはアプリケーションのメイン ヘッダー ファイルです。このファイルには、
   Resource.h を含む、その他のプロジェクト固有のヘッダーが含まれていて、
   CHistoricalDataConverterApp アプリケーション クラスを宣言します。

HistoricalDataConverter.cpp
   これは、アプリケーション クラス CHistoricalDataConverterApp を含むメイン アプリケー
   ション ソース ファイルです。

HistoricalDataConverter.rc
...
res\HistoricalDataConverter.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル HistoricalDataConverter.rc に含まれていま
   す。

res\HistoricalDataConverter.rc2
...
HistoricalDataConverterDlg.h, HistoricalDataConverterDlg.cpp - ダイアログ
   これらのファイルは CHistoricalDataConverterDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な HistoricalDataConverter.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   HistoricalDataConverter.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
...
HistoricalDataConverter.manifest
